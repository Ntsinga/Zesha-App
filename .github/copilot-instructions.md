...
- When identifying a possible frontend/backend contract gap or routing caveat, verify the controlling code on both sides before presenting it as a caveat. Do not leave a statement like "if this fails, the backend may not be setting X" unverified when the relevant backend/frontend code is available.
